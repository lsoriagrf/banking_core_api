...
public class AccountEventKafkaConsumer {
    ...
    private final KafkaReceiver<String, String> accountEventsReceiver;
    private final AccountEventHandlerPortIn accountEventHandler;
    ...
    @PostConstruct
    void start() {
        subscription = accountEventsReceiver.receive()
    ...
    private Mono<Void> process(ReceiverRecord<String, String> record) {
        return Mono.fromCallable(() -> jsonMapper.readValue(record.value(), AccountEvent.class))
                .flatMap(accountEventHandler::handle)
                .onErrorResume(error -> {
                    log.error("Discarding account event at offset {}: {}",
